                ...
                <tbody>
                  ${filteredApps.map(app => {
                    ...
                    <tr style="${isPending ? 'background:rgba(217, 119, 6, 0.04);' : ''}">
                      ...
                      <td>
                        <button class="btn ${isPending ? 'primary' : 'ghost-sm'}" onclick="window.openPoliceReviewComplaintModal('${app.id}')" style="padding:5px 12px;font-size:11.5px;font-weight:600;">
                          ${isPending ? '⚡ Review Application' : 'Inspect &amp; Update'}
                        </button>
                      </td>
                    </tr>
                  `;}).join('')}
...
export function viewCaseDetail(caseId, tab) {
  ...
  let body = '';
  if (tab === 'overview') body = caseOverviewTab(c);
  ...
  return `
    ...
    <div class="page-title">
      <h2>${escapeHtml(c.title)} <span class="badge ${c.status}" style="vertical-align:middle;margin-left:8px;">${c.status}</span></h2>
      <div class="actions">
        <select id="statusSelect" class="raised-sm" style="box-shadow:none;padding:9px 12px;border-radius:11px;">
          ${[
            ...
            .map(
              s =>
                `<option value="${s.value}" ${(s.value === c.status || (s.value === 'closed' && c.status === 'resolved')) ? 'selected' : ''}>${s.label}</option>`
            )
            .join('')}
        </select>
      </div>
...
export function caseReportTab(c) {
  ...
  <div class="toolbar" style="margin-top:16px;"><button class="btn" id="copyReportBtn" data-case="${c.id}">Copy Report</button><button class="btn" id="printReportBtn">Print / Save PDF</button></div>
  `;
}
