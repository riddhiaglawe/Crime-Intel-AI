...
import {
  ...
  citizenEvidenceCaseFilter,
  citizenSuspectCaseFilter
} from './citizen.js';
...
import {
  acceptLegalProvision,
  rejectLegalProvision,
  openEditLegalProvisionModal,
  openAddCustomLegalProvisionModal,
  saveModifiedLegalProvision,
  triggerReanalyzeProvisions,
  generateComplaintAssessmentPdf
} from './legal-provisions.js';
...
export function wireDynamicHandlers() {
  ...
  // Citizen app status dropdown (in case detail view for citizen complaints)
  const citizenAppStatusSelect = document.getElementById('citizenAppStatusSelect');
  if (citizenAppStatusSelect) {
    citizenAppStatusSelect.addEventListener('change', async e => {
      const newStatus = e.target.value;
      const officerName = DATA.meta.analyst || 'Investigating Officer';
      await updateApplicationStatus(route.id, newStatus, `Status updated to ${newStatus}`, '', officerName);
...
function setupModalListeners() {
  ...
  if (searchInput) {
    searchInput.addEventListener('input', e => {
      ...
      const hitCase = allCases.find(c => {
        if (!c) return false;
        
        // 1. Core text and identifiers matching
        if ((c.title || '').toLowerCase().includes(q)) return true;
        if ((c.description || '').toLowerCase().includes(q)) return true;
        if ((c.num || '').toLowerCase().includes(q)) return true;
        if ((c.id || '').toLowerCase().includes(q)) return true;
        
        // 2. Citizen app complainant fields matching
        if (c.isCitizenApp) {
          if ((c.citizenName || '').toLowerCase().includes(q)) return true;
        ...
        if (Array.isArray(c.evidence)) {
          if (c.evidence.some(ev => 
            (ev.text || '').toLowerCase().includes(q) || 
            (ev.location || '').toLowerCase().includes(q) ||
            (Array.isArray(ev.tags) && ev.tags.some(t => t.toLowerCase().includes(q)))
          )) return true;
        }

        // 4. Suspect matches
        if (Array.isArray(c.suspects)) {
          if (c.suspects.some(s => 
...
window.openCitizenUploadEvidenceModal = function() {
  ...
  if (select) {
    select.innerHTML = activeApps.map(a => `
      <option value="${a.id}" ${citizenEvidenceCaseFilter === a.id ? 'selected' : ''}>${escapeHtml(a.num || a.id)} - ${escapeHtml(a.title)}</option>
...
window.openAddSuspectModal = function() {
  ...
  if (select) {
    select.innerHTML = activeApps.map(a => `
      <option value="${a.id}" ${citizenSuspectCaseFilter === a.id ? 'selected' : ''}>${escapeHtml(a.num || a.id)} - ${escapeHtml(a.title)}</option>
