 ...
 */
export function computeJaccard(tagsA = [], tagsB = []) {
 ...
 */
export function computeCosine(tagsA = [], tagsB = []) {
  const setA = new Set(tagsA);
  const setB = new Set(tagsB);
  const intersection = new Set([...setA].filter(x => setB.has(x)));
  const magA = Math.sqrt(setA.size);
  const magB = Math.sqrt(setB.size);
  return magA * magB === 0 ? 0 : intersection.size / (magA * magB);
}

/**
 * Jaccard-Cosine Relationship: JCOS(A,B) = Jaccard(A,B) × Cosine(A,B)
 */
export function computeJCOS(tagsA = [], tagsB = []) {
  return computeJaccard(tagsA, tagsB) * computeCosine(tagsA, tagsB);
}

export function computeSuspectScore(suspect, evidenceList = []) {
  let matches = [];
  evidenceList.forEach(ev => {
    const shared = ev.tags ? ev.tags.filter(t => suspect.tags && suspect.tags.includes(t)) : [];
    if (shared.length > 0) {
      matches.push({ evidenceId: ev.id, shared });
    }
  });

  const E = evidenceList.length;
  const M = matches.length;
  let score = 8;

  if (E > 0) {
    const denominator = M + ALPHA * (E - M);
    score = denominator > 0 ? 8 + 92 * (M / denominator) : 8;
  }
